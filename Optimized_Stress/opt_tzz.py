...
Data= pd.read_csv("K:\LMC\Sanjay\Code\Optimization\optimization_stress\Optimized_stress_const_RR\\stress_hyperperameter_ZZ2.csv")
...
t_z_s0=tw*Data['zeta_z']*Data['A(w)']*jv(Data['alpha_z'],KS0*Data['gamma_z']*a)*(KS0*a)**Data['beta_z']
#
# t_r_s0=Data['tr']*Data['A(w)']*jv(1,KS0*Data['gamma']*a)
t_z_s0_mean=tw*Data_mean['zeta_z']*Data['A(w)']*jv(Data_mean['alpha_z'],KS0*Data_mean['gamma_z']*a)*(KS0*a)**Data_mean['beta_z']

t_z_a0=tw*Data['zeta_z']*Data['A(w)']*jv(Data['alpha_z'],KA0*Data['gamma_z']*a)*(KA0*a)**Data['beta_z']

t_z_a0_mean=tw*Data_mean['zeta_z']*Data['A(w)']*jv(Data_mean['alpha_z'],KA0*Data_mean['gamma_z']*a)*(KA0*a)**Data_mean['beta_z']
...
graph.figureplot(Data['Freq[Hz]'],Data['alpha_z'],ax=axes[0], linestyle='--',marker='o',label=r'$\alpha_z$',c='k', title='Hyper-Parameters-'+r'$\tau_{zz}$', markersize=2)


# fig,axes=plt.subplots(1,1)
graph.figureplot(Data['Freq[Hz]'],Data['gamma_z'],ax=axes[1], linestyle='--',marker='o',label=r'$\gamma_z$',c='r', markersize=2)

# fig,axes=plt.subplots(1,1)
graph.figureplot(Data['Freq[Hz]'],Data['beta_z'],ax=axes[2], linestyle='--',marker='o',label=r'$\beta_z$',c='b', markersize=2)
graph.figureplot(Data['Freq[Hz]'],Data['zeta_z'],ax=axes[3], linestyle='--',marker='o',label=r'$\zeta_z$',c='g', markersize=2,path=pathsave,filename='hyper_tzz')
